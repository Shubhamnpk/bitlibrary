...
import { parseArticleXml } from './api/lib/article-xml';
...
const renderBioCArticleHtml = (body: string, imageMap: Record<string, string> = {}): string | null => {
...
const readerMessageHtml = (message: string) => `<!doctype html><html><head><meta charset="utf-8"><style>
  :root{color-scheme:dark;background:#09090b;color:#f4f4f5}
...
const readerTextHtml = async (body: string, contentType: string, target: URL) => {
  ...
  return (useBioCRenderer ? renderBioCArticleHtml(body, imageMap) : null)
...
const readerHtmlDocument = (body: string, targetUrl: string) => {
...
const sendProxyMessage = (res: any, statusCode: number, message: string, isReaderRequest: boolean) => {
  res.statusCode = statusCode;
  if (isReaderRequest) {
    res.setHeader('content-type', 'text/html; charset=utf-8');
    res.end(readerMessageHtml(message));
  } else {
    res.end(message);
  }
};
          ...
          configureServer(server) {
            ...
            server.middlewares.use('/api/pdf-proxy', async (req, res) => {
              try {
                ...
                if (!rawUrl) {
                  sendProxyMessage(res, 400, 'Missing url parameter.', isReaderRequest);
                  return;
                }

                const target = new URL(rawUrl);
                if (!['http:', 'https:'].includes(target.protocol)) {
                  sendProxyMessage(res, 403, `Reader URL protocol is not allowed: ${target.protocol}`, isReaderRequest);
                  return;
                }
                if (isPrivateReaderHostname(target.hostname)) {
                  sendProxyMessage(res, 403, 'Reader URL host is private or local.', isReaderRequest);
                  return;
                }
                ...
                if (!upstream.ok || !upstream.body) {
                  sendProxyMessage(res, upstream.status || 502, 'Unable to fetch reader resource.', isReaderRequest);
                  return;
                }
                ...
                if (!normalizedContentType.includes('pdf') && !isPdfUrl && (!isReaderRequest || !isSupportedReaderContent)) {
                  sendProxyMessage(res, 415, isReaderRequest ? 'Target is not a supported reader resource.' : 'Target is not a PDF.', isReaderRequest);
                  return;
                }
                ...
                if (isReaderRequest && (normalizedContentType.includes('xml') || normalizedContentType.includes('text/plain') || isXmlUrl || isTextUrl)) {
                  ...
                  res.end(await readerTextHtml(body, contentType, target));
                  return;
                }
                ...
                if (isReaderRequest && (normalizedContentType.includes('text/html') || normalizedContentType.includes('application/xhtml'))) {
                  ...
                  res.setHeader('cache-control', 'public, max-age=3600');
                  res.setHeader('access-control-allow-origin', '*');
                  res.end(readerHtmlDocument(body, target.toString()));
                  return;
                }

                res.statusCode = upstream.status;
                res.setHeader('content-type', isPdfUrl && !normalizedContentType.includes('pdf') ? 'application/pdf' : contentType);
                res.setHeader('cache-control', 'public, max-age=3600');
                res.setHeader('access-control-allow-origin', '*');
                const upstreamHonoredRange = !range || upstream.status === 206;
                res.setHeader('accept-ranges', upstreamHonoredRange ? upstream.headers.get('accept-ranges') || 'bytes' : 'none');
                ...
                res.statusCode = 500;
                res.setHeader('content-type', 'text/html; charset=utf-8');
                res.end(readerMessageHtml('Reader proxy failed.'));
              }
            });
