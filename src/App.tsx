...
import { searchBooksInGutendex, fetchBooksFromGutendex } from '@/services/bookService';
    ...
    <div className="min-h-screen bg-bit-bg text-bit-text font-sans selection:bg-bit-accent selection:text-black">
      ...
      <main className={`pb-20 px-6 max-w-7xl mx-auto min-h-screen flex flex-col relative z-0 ${isReader ? '' : 'pt-24'}`}>
        ...
        <Routes>
           ...
           <Route path="/book/:id" element={<BookDetailsRoute allBooks={featuredBooks} onRead={(id) => navigate(`/reader/${id}`)} onBookClick={(id) => navigate(`/book/${id}`)} />} />
...
const BookDetailsRoute: React.FC<{allBooks: Book[], onRead: (id: string) => void, onBookClick: (id: string) => void}> = ({allBooks, onRead, onBookClick}) => {
    const { id } = useParams();
    const navigate = useNavigate();
    ...
    if (loading) return <div className="py-40 text-center animate-pulse font-mono text-bit-accent">ACCESSING DATA CONE...</div>;
    if (!book) return <div className="py-40 text-center font-mono text-red-500">ERROR: SECTOR {id} NOT FOUND</div>;
...
const ReaderRoute: React.FC = () => {
    ...
    if (loading) return <div className="py-40 text-center animate-pulse font-mono text-bit-accent">SYNCHRONIZING STREAM...</div>;
    if (!book) return <div className="py-40 text-center font-mono text-red-500">ERROR: NO STREAM AT {id}</div>;

    return <Reader book={book} onClose={() => navigate(-1)} />;
};
