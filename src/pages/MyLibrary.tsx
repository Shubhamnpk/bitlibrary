...
import { ArrowRight, Bookmark, BookOpenText, ChevronDown, Clock, Database, HardDrive, Headphones, History, Library as LibraryIcon, Moon, Search, Settings, ShieldCheck, Sun, Trash2, User2 } from 'lucide-react';
...
const MyLibrary: React.FC<MyLibraryProps> = ({
  ...
  const handleClearStaleCaches = () => {
    clearStaleCaches();
    refreshStorageReport();
  };

  const handleClearRecoverableCaches = () => {
    clearRecoverableCaches();
    refreshStorageReport();
  };

  const handleClearReaderProgress = () => {
    clearStorageCategory('reader');
    refreshStorageReport();
  };
  ...
  const toolSections = [
    ...
    },
    {
      ...
      summary: `${formatStorageBytes(storageSummary.totalBytes)} on this device`,
      icon: HardDrive,
      open: showStorage,
      setOpen: setShowStorage,
      content: (
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-2">
            <button
              type="button"
              onClick={handleClearStaleCaches}
              disabled={storageSummary.staleCount === 0}
            ...
            <button
              type="button"
              onClick={handleClearRecoverableCaches}
            ...
            >
              ...
              <span className="truncate">Caches</span>
            </button>
            <button
              type="button"
              onClick={handleClearReaderProgress}
          ...
          <p className="text-xs leading-6 text-bit-muted">
            Saved books, audiobooks, profile name, and theme are protected from cache cleanup.
          </p>
        </div>
      ),
    },
